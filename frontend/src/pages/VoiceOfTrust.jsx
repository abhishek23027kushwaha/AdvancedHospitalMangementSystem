  ...
  <div className="flex gap-0.5">
    {[1, 2, 3, 4, 5].map((s) => (
      <Star
        key={s}
        size={14}
        className={s <= rating ? "text-[#F59E0B] fill-[#F59E0B]" : "text-gray-200 fill-gray-200"}
      />
    ))}
  </div>
);

const ReviewCard = ({ name, role, rating, avatar, review }) => (
  <div className="bg-[#F8FAFC] rounded-2xl p-5 border border-[#E2E8F0] mb-4 flex-shrink-0 relative group hover:shadow-md transition-shadow">
    <div className="flex items-center justify-between mb-3">
      <div className="flex items-center gap-3">
        <div className="w-11 h-11 rounded-full flex items-center justify-center text-xl bg-[#EFF6FF] border border-[#DBEAFE]">
          {avatar}
        </div>
        <div>
          <p className="font-bold text-sm text-[#0F172A]">{name}</p>
          <p className="text-[#64748B] text-xs font-semibold">{role}</p>
        </div>
      </div>
    ...
    <div className="relative">
      <Quote size={14} className="text-[#2563EB] opacity-40 absolute top-0.5 left-0" />
      <p className="text-[#64748B] text-sm leading-relaxed italic pl-5">{review}</p>
    </div>
  </div>
);

const ScrollColumn = ({ reviews, label, icon, duration }) => {
  const doubled = [...reviews, ...reviews];
  return (
    <div className="flex-1 rounded-3xl overflow-hidden p-5 bg-[#FFFFFF] border border-[#E2E8F0] shadow-sm">
      {/* Column Header */}
      <div className="text-center mb-5 pb-4 border-b border-[#E2E8F0] flex items-center justify-center gap-2">
        <span className="text-xl">{icon}</span>
        <span className="font-black text-lg text-[#0F172A]">{label}</span>
      </div>

      {/* Scrolling Content */}
      <div className="overflow-hidden h-[520px] relative">
        {/* Top fade */}
        <div className="absolute top-0 left-0 right-0 h-12 z-10 bg-gradient-to-b from-[#FFFFFF] to-transparent pointer-events-none" />
        {/* Bottom fade */}
        <div className="absolute bottom-0 left-0 right-0 h-12 z-10 bg-gradient-to-t from-[#FFFFFF] to-transparent pointer-events-none" />
